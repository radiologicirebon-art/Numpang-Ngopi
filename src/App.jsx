  ...
  const svg = `
    <svg xmlns='http://www.w3.org/2000/svg' width='800' height='600'>
      ...
      <g transform='translate(260,100)'>
        <text x='0' y='40' font-size='38' font-family='Segoe UI, Roboto, Arial' fill='#3b3b3b'>${escapeXml(
          text.split(' ')[0] || text,
        )}</text>
        <text x='0' y='80' font-size='20' font-family='Segoe UI, Roboto, Arial' fill='#555'>${escapeXml(
          text,
        )}</text>
      </g>
    </svg>`
...
async function patchOrder(key, data) {
  const res = await fetch(`${DB_URL}/orders/${key}.json`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
...
export default function App() {
  const [cart, setCart] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('numpang_cart')) || []
    } catch {
      return []
    }
  })
  ...
  function toggleAddon(i, a) {
    setCart((p) => {
      const c = [...p]
      const f = c[i].addons || []
      const pos = f.indexOf(a.id)
      if (pos === -1) f.push(a.id)
      else f.splice(pos, 1)
      c[i].addons = f
      return c
    })
  ...
  function itemSubtotal(ci) {
    const addonSum = (ci.addons || []).reduce(
      (s, aId) => s + (MENU.Addons.find((x) => x.id === aId)?.price || 0),
      0,
    )
  ...
  function buildMessage() {
    if (!cart.length) return 'Halo saya ingin pesan, (belum ada item).'
    const lines = ['Halo, saya ingin pesan:']
    cart.forEach((ci) => {
      const addons = (ci.addons || [])
        .map((aId) => MENU.Addons.find((a) => a.id === aId)?.name)
        .filter(Boolean)
      lines.push(
        `- ${ci.name} x${ci.qty}${
          addons.length ? ` [${addons.join(', ')}]` : ''
        } = ${currency(itemSubtotal(ci))}`,
      )
    })
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-3 py-1 rounded ${
                screen === 'pos' ? 'bg-blue-600 text-white' : 'border'
              }`}
            >
              POS
            </button>
            <button
              onClick={() => setScreen('barista')}
              className={`px-3 py-1 rounded ${
                screen === 'barista' ? 'bg-yellow-500 text-white' : 'border'
              }`}
            >
              Display Barista
            </button>
            <button
              ...
              className={`px-3 py-1 rounded ${
                screen === 'cooking' ? 'bg-orange-500 text-white' : 'border'
              }`}
            >
              Display Cooking
            </button>
            <button
              ...
              className={`px-3 py-1 rounded ${
                screen === 'owner' ? 'bg-green-600 text-white' : 'border'
              }`}
            >
              Laporan Owner
            </button>
          ...
          <div className="bg-white rounded-2xl shadow p-6 grid md:grid-cols-2 gap-6">
            ...
            <div>
              <p className="text-sm text-gray-600 mb-4">
                Pilih menu, atur jumlah dan addons. Pesanan disimpan realtime &
                dikirim ke WhatsApp.
              </p>
              {Object.entries(MENU)
                .filter(([k]) => k !== 'Addons')
                  ...
                  <div key={group} className="mb-6">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {items.map((it, idx) => (
                        ...
                        >
                          <img
                            src={makeIllustration(it.name, idx + group.length)}
                            alt={it.name}
                            className="menu-img"
                          />
                          <div className="p-3 flex flex-col flex-1">
                            ...
                            <button
                              ...
                              className="mt-auto px-3 py-1 bg-blue-600 text-white rounded"
                            >
                              Tambah
                            </button>
                          </div>
              ...
              <div className="mt-4 text-xs text-gray-500">
                Addons: Creamy (1K), Aren (1K), Es Batu (1K).
              </div>
            </div>
            ...
            <div>
                ...
                {cart.map((ci, idx) => (
                  <div key={idx} className="border rounded p-3">
                    ...
                    <div className="flex gap-2 mt-1 flex-wrap">
                      {MENU.Addons.map((ad) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={(ci.addons || []).includes(ad.id)}
                            onChange={() => toggleAddon(idx, ad)}
                          />
                          <span>
                            {ad.name} (+{currency(ad.price)})
                          </span>
                        </label>
                      ))}
                    </div>
                    <div className="mt-2 flex justify-between items-center">
                      <div className="text-sm text-gray-600">
                        Subtotal: {currency(itemSubtotal(ci))}
                      </div>
                      <button
              ...
              <div className="mt-4 border-t pt-4">
                ...
                <div className="mt-4 flex gap-2">
                  ...
                  >
                    {sending ? 'Mengirim...' : 'Kirim ke WhatsApp & Simpan'}
                  </button>
                  <button
                    onClick={() => {
                      setCart([])
                      setLastResult(null)
                      localStorage.removeItem('numpang_cart')
                    }}
                ...
                {lastResult && (
                  <div
                    className={`mt-3 text-sm ${
                      lastResult.ok ? 'text-green-700' : 'text-red-700'
                    }`}
                  >
                    {lastResult.ok
                      ? `Disimpan ke Firebase`
                      : `Gagal simpan: ${
                          lastResult.error || JSON.stringify(lastResult.resp)
                        }`}
                  </div>
                )}
              </div>
